                    ...
                    if (roll <= needed) {
                        ...
                        if (current === 0) {
                            if (system === "Shield Generator") {
                                ...
                                state.Valor.shipState[this.id].shields = cs;
                                this.SetShields(-cs);
                            }
...
log(status)
        ...
        StartTurn() {
            ...
            SetupCard(this.name,"",this.nation);
            ...
            state.Valor.shipState[this.id].targets = [];
                    ...
                    if (this.token.get(SM.nobridge) || this.token.get(SM.ooc)) {
                        ...
                        if (damagedSystems.length > 1) { 
                            ...
                            _.each(damagedSystems,system => {
                                let rep = state.Valor.shipState[this.id].systemRepairs[system] || 0;
                                let button = system;
                                if (rep > 0) {
        ...
        RepairSystem(system,assignedDCT) {
            ...
            let bonus = 0;
            if (state.Valor.shipState[this.id].systemRepairs[system]) {
                bonus = state.Valor.shipState[this.id].systemRepairs[system];
            }
            ...
            if (repairRoll > needed) {
                ...
                state.Valor.shipState[this.id].systemRepairs[system] = (bonus + 1);
            } else {
                let add = "";
                ...
                if (sys) {  
                    if (system === "Fire Control" || system === "Shield Generator") {
                        ...
                        if (current === 0 && system === "Shield Generator") {
                            let shields = state.Valor.shipState[this.id].shields;
                ...
                this.damagedSystems = damagedSystems;
                AttributeSet(this.charID,"damagedsystems",damagedSystems.toString());
                state.Valor.shipState[this.id].systemRepairs[system] = 0;
...
log(dt)
    ...
    const CreateHelmAbility = (ship) => {
        let helmID = state.Valor.shipState[ship.id].helmID;
        if (helmID) {
        ...
        state.Valor.shipState[ship.id].helmID = helmID;
    }
...
const Move = (msg) => {
    ...
    SetupCard(ship.name,"Helm",ship.nation);
...
//blurbs
    ...
    const Orders = (msg) => {
        ...
        SetupCard(ship.name,order,ship.nation);
...
log("Here")
            ...
            let roll = randomInteger(6);
            let mod = state.Valor.shipState[ship.id].emergencyThrusts;
log(mod)
            roll += mod;
            let which = "";
            if (roll < 6) {
            ...
            state.Valor.shipState[ship.id].emergencyThrusts++;
        }
        ...
        if (order === "Engineering") {
            ...
            if (suborder === "Repairs") {
                ...
                state.Valor.shipState[id].damageControl = suborder;
...
//["!Helm","-OzcDRbtvA4HNItU93Vy","6","Increase Speed by 2","Ahead"]
        ...
        SetupCard(ship.name,"Impulse",ship.nation);
...
outputCard.body.push("Path: " + path.toString());
    ...
    const RepBack = (msg) => {
        ...
        let ship = ShipArray[Tag[1]];
        let system = Tag[2];
        SetupCard(ship.name,"",ship.nation);
    ...
    const Fire = (msg) => {
        ...
        SetupCard(shooter.name,"Tactical Station",shooter.nation);
...
log("Adv?  " + advFC)
        ...
        if (availableFC === 0) {
            ...
            let targetsFiredOn = state.Valor.shipState[shooter.id].targets;
...
log(weapon)
        ...
        if (state.Valor.shipState[shooter.id].targets.includes(target.id) === false) {
            state.Valor.shipState[shooter.id].targets.push(target.id);
        }
    ...
    const Combat = () => {
        ...
        if (currentTurnItem.custom === "Turn") {
            state.Valor.turn = currentTurnItem.pr
        }
        //ping model's token
        if (ship) {
            if (ship.token) {
            ...
            SetupCard("Turn " + state.Valor.turn,"","Neutral");
    ...
    const Warp = (msg) => {
        ...
        let ship = ShipArray[id];
        if (ship) {
            SetupCard(ship.name,"Warp",ship.nation);
